...
function loadGoogleFonts (fontFamilyArray, callback, failedCallback) {
  if(fontFamilyArray.length === 0) return;
  WebFont.load({
    google: {
      families: fontFamilyArray,
    },
    //what should be done if this function succeeds?
    active: callback,
    //what should be done if this function fails?
    inactive: (failedCallback == undefined)
    ? function () {
        //provide a default value for failedCallback so it doesn't always have to be defined
        console.log("Font(s) \'"+fontFamilyArray+"\' could not be loaded.");
        wickEditor.alertbox.showMessage("Loading failed.", 3000);
    }
    : failedCallback,
  });
}
