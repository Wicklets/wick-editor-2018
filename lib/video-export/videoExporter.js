var VideoExporter = function () {
    ...
    worker.onmessage = function (event) {
      ...
      if (message.type == "ready") {
        ...
        buffers.forEach(function(file) {
          if (message.commandName in self.commandCallbacks) {
            // If the user gave us a callback, use that.
            console.log(self.commandCallbacks)
            console.log(message.commandName)
            self.commandCallbacks[message.commandName](file.data); 
          } else {
            // Otherwise just download the file
            self.downloadFile(file.data, 'video.mp4'); 
          }
        });
      };
    };
  ...
  this.runCommand = function(command, fileNames, fileBuffers, commandName, commandCallback) {
        ...
        self.commandCallbacks[commandName] = commandCallback; 
