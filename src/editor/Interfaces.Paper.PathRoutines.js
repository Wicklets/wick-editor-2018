...
var PathRoutines = function (paperInterface, wickEditor) {
    ...
    self.fill = function (x,y,color) {
        ...
        touchingPaths.forEach(function (path) {
            if(path.paper.contains(new paper.Point(x,y))) {
                self.setFillColor([path], color);
            }
        });
        ...
        holes = [];
        hitPaths = [];
        superPath._children.forEach(function (path) {
        ...
        hitPaths.sort(function (a,b) {
            return Math.abs(a.area) > Math.abs(b.area);
        });
        var hitPath = hitPaths[0];

        var path = hitPath;

        holes.forEach(function (hole) {
            //console.log(hole)
            //path = hole.subtract(path);
        });
        ...
        var superPathString = group.exportSVG({asString:true});
        var svgString = '<svg id="svg" version="1.1" width="'+group.bounds._width+'" height="'+group.bounds._height+'" xmlns="http://www.w3.org/2000/svg">' +superPathString+ '</svg>'
        var superPathWickObject = WickObject.createPathObject(superPathString);
        superPathWickObject.x = group.position.x;
        superPathWickObject.y = group.position.y;
